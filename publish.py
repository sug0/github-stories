...
class Publisher(object):
    ...
    def publish(self, content=None, remove=False):
        if not content:
            content = '.tmp'
            remove = True
        ...
        lock = self._pathtofile('publish.lock')

        if os.system(f'cp {content} {path}') != 0:
            raise PublishError('cp failed')
        if os.system(f'git add {path} {lock} && git commit -m "{ctime()}"') != 0:
            raise PublishError('git failed')
        if remove:
            os.remove(content)
